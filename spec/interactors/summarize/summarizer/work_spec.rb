...
describe Summarize::Summarizer::Work do
  ...
  it 'sums up each work time type' do
    FactoryGirl.create(:time_entry, starts: '2013-04-24 9:00:00', ends: '2013-04-24 18:00:00', time_type: :work, user: user)
  ...
  describe 'adjustments' do
    context 'adjustment is for vacation' do
      let!(:adjustment) { FactoryGirl.create(:adjustment, user: user, time_type: TEST_TIME_TYPES[:vacation], duration: 1.0.work_days, date: range.min) }
    ...
    context 'adjustment is not for vacation' do
      let!(:adjustment) { FactoryGirl.create(:adjustment, user: user, time_type: TEST_TIME_TYPES[:paid_absence], duration: 1.0.work_days, date: range.min) }
