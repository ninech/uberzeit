...
describe AbsenceSchedule do
  ...
  context 'validates all the things!' do
    ...
    it 'requires a valid date when ending condition is set to date' do
      ...
      FactoryGirl.build(:absence_schedule, :active, ends_date: nil).should_not be_valid
      FactoryGirl.build(:absence_schedule, :active, ends_date: '1000 B.C.').should_not be_valid
      FactoryGirl.build(:absence_schedule, :active, ends_date: '2013-07-20').should be_valid
    end

    it 'enforces that the end date is on or after the end date of the associated absence'  do
      FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: absence.end_date - 1.day).should_not be_valid
    end
  end
  ...
  context '#occurrences' do
    ...
    it 'ignores the repeating character of an absence when the ends date is BEFORE the end date of the absence' do
      absence_schedule = FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: absence.end_date - 42.days)

      occurrences = absence_schedule.occurrences('2013-01-01'.to_date..'2013-12-31'.to_date)
      occurrences.should eq([absence.start_date..absence.end_date])
    end

    context 'with weekly repeats' do
      it 'finds the occurrences for the specified interval' do
        absence_schedule = FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: '2013-05-23', weekly_repeat_interval: 2)
      ...
      it 'finds occurrences which span multiple days' do
        ...
        absence_schedule = FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: '2013-03-21')
        absence_schedule.occurrences(date).should_not be_empty
    ...
    context 'ending condition' do
      it 'ends on a specific date' do
        absence_schedule = FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: '2013-07-18')
        absence_schedule.occurrences(date).should_not be_empty
    ...
    context '#occurring?' do
      it 'returns a boolean indicating whether or not there are occurrences in the supplied date/range' do
        absence_schedule = FactoryGirl.build(:absence_schedule, :active, absence: absence, ends_date: '2013-05-23')
