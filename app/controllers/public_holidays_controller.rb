class PublicHolidaysController < ApplicationController
  ...
  def create
    ...
    if @public_holiday.save
      redirect_to public_holidays_path, :notice => t('model_successfully_created', model: PublicHoliday.model_name.human)
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @public_holiday.update_attributes(params[:public_holiday])
      redirect_to public_holidays_path, :notice => t('model_successfully_updated', model: PublicHoliday.model_name.human)
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    ...
    if @public_holiday.destroy
      redirect_to public_holidays_path, :notice => t('model_successfully_deleted', model: PublicHoliday.model_name.human)
    else
      render :action => 'edit'
    end
  end
