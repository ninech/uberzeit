class Ability
  ...
  def initialize(user)
    if user
      ...
      can :manage, Activity, user_id: user.id, locked: false
      ...
      if user.team_leader?
        ...
        can :read, Activity, user_id: manageable_user_ids(user)
        can :manage, Activity, user_id: manageable_user_ids(user), locked: false
      end
