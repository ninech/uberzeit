module Dated
  ...
  module ClassMethods
    def scope_date(date_field_name)
      [:add_date_between_scope, :add_year_month_scope, :add_year_scope].each do |method|
    ...
    def add_date_between_scope(date_field_name)
      class_eval do
        scope :"with_#{date_field_name}", ->(date_range) do
    ...
    def add_year_month_scope(date_field_name)
      class_eval do
        scope :"with_#{date_field_name}_in_year_and_month", ->(year, month) do
          send(:"with_#{date_field_name}", UberZeit.month_as_range(year, month))
        end
      end
    end
    ...
    def add_year_scope(date_field_name)
      class_eval do
        scope :"with_#{date_field_name}_in_year", ->(year) do
          send(:"with_#{date_field_name}", UberZeit.year_as_range(year))
        end
      end
    end
  end
