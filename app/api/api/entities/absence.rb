...
class API::Entities::Absence < Grape::Entity
  expose :id
  ...
  expose :time_type, using: API::Entities::TimeType, if: ->(*args) { embed_time_type?(*args) }
  expose :time_type_id, unless: ->(*args) { embed_time_type?(*args) }

  expose :user, using: API::Entities::User, if: ->(*args) { embed_user?(*args) }
